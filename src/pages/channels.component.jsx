...
//redux
import { setProductsTableData } from '../redux/product/productActions'
...
const Channels = () => {
    ...
    const productsTableData = useSelector(state => state.product.productsTableData)
    const [isModalOpen, setModalOpen] = useState(false)
    const data = useMemo(()=> productsTableData, [productsTableData])
    const optionsSVG =
    ...
    const columns = useMemo(()=> [
      ...
      },
      {
        ...
        Cell: ({ row })=> {
         return (
          <ActionsMenu rowData={row} productsActions={true} />
    ...
    const instance = useTable({ columns, data },  useGlobalFilter, usePagination, useRowSelect)

    useEffect(()=> {
      axios
          .get('http://localhost:3000/products')
          .then(res=> dispatch(setProductsTableData(res.data)))
    ...
    return (
        <div className='flex flex-col space-y-12 px-6 py-12 sm:overflow-x-hidden'>
          <CustomTable 
